 ...
 */
export function safeParseWithMcpError<T extends z.ZodTypeAny>(
  schema: T,
  data: unknown
): z.infer<T> {
 ...
 * }
 */
export function handleMcpError(error: unknown, toolName?: string): never {
  ...
  if (error instanceof z.ZodError) {
    ...
    throw new McpError(
      ErrorCode.InvalidParams,
      `${prefix}Invalid parameters: ${errorMessages}`
    );
  }
...
export function createEnumSchema<T extends readonly [string, ...string[]]>(
  ...
  return z.enum(values as any).describe(description);
}
 ...
 */
export function createOptionalWithDefault<T extends z.ZodTypeAny, D>(
  baseSchema: T,
  defaultValue: D,
  description: string
): z.ZodDefault<T> {
 ...
 */
export function formatJsonResponse(data: unknown, indent = 2): string {
 ...
 */
export function createStructuredResponse(text: string, structuredContent: unknown) {
  return {
    content: [{ type: 'text' as const, text }],
    structuredContent,
  };
